 ...
 */
	if(!function_exists('wd_custom_products_category_function')){
		function wd_custom_products_category_function($atts,$content){
			...
			if ( ! $category ) return;
			wp_reset_query(); 
			global $featured_datas,$data,$product,$post;
			...
			if(isset($data['wd_prod_cat_column']) && absint($data['wd_prod_cat_column']) > 0 ){
				$_old_wd_prod_cat_column = $data['wd_prod_cat_column'];
				$data['wd_prod_cat_column'] = 3;
			} 
			...
			?>
					...
					<?php
						...
						if($_count == 0){
							...
							$old_data = $data['wd_prod_cloudzoom'];
							$data['wd_prod_cloudzoom'] = 0;
							if(!isset($_featured_prod)){
							...
							<?php 
								global $data;
								if(!isset($data['wd_catelog_mod']) || (isset($data['wd_catelog_mod']) && $data['wd_catelog_mod'] == 1)){
							?>
							...
							$data['wd_prod_cloudzoom'] = $old_data;
			...
			if(isset($_old_wd_prod_cat_column) && absint($_old_wd_prod_cat_column > 0 )){
				$data['wd_prod_cat_column'] = $_old_wd_prod_cat_column  ;
			}
	...
	if(!function_exists('wd_custom_products_category_style2_function')){
		function wd_custom_products_category_style2_function($atts,$content){
			...
			global $woocommerce, $woocommerce_loop,$data;
			if ( empty( $atts ) ) return;
			extract( shortcode_atts( array(
				'per_page' 		=> '4'
				,'orderby'   	=> 'title'
	...
	if(!function_exists('wd_custom_products_category_grid_function')){
		function wd_custom_products_category_grid_function($atts,$content){
			...
			if(isset($data['wd_prod_cat_column']) && absint($data['wd_prod_cat_column']) > 0 ){
				$_old_wd_prod_cat_column = $data['wd_prod_cat_column'];
				$data['wd_prod_cat_column'] = $columns;
			} 

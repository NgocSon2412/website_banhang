...
<form id="contact-form-<?php echo $form_name ?>" name="contact-form-<?php echo $form_name ?>" class="contact-form<?php ( !$is_footer ? ' row-fluid' : '' )?>" method="post" action="" enctype="multipart/form-data">
        ...
        <ul>
            ...
            foreach ( $fields as $id => $field ) {
                ...
                if ( isset( $field['data_name'] ) && ( $error || empty( $user_message ) ) )
                    $value = ( isset( $_POST['yit_contact'][$field['data_name']] ) ) ? $_POST['yit_contact'][$field['data_name']] : '';
            ...
            <li class="submit-button col-sm-<?php echo $max_width?> no-padding-right">
                <div style="position:absolute; z-index:-1; <?php  echo (is_rtl() ? "margin-right:-9999999px;" : "margin-left:-9999999px;")  ;?> "><input type="text" name="yit_bot" id="yit_bot"/></div>
                <input type="hidden" name="yit_action" value="sendemail" id="yit_action"  />
                <input type="hidden" name="yit_referer" value="<?php echo esc_attr( yit_curPageURL() ) ?>" />
                <?php if( $do_ajax == 1 ): ?>
                    <input type="hidden" name="yit_ajax" class="yit_ajax" value="<?php echo esc_attr( $do_ajax ) ?>" data-action="yit_contact_form_submit"/>
                <?php endif; ?>
                <?php
                ...
                <input type="submit" name="yit_sendemail" value="<?php echo YIT_Contact_Form()->get('_submit_label') ?>" class="yit_sendemail <?php echo $button_style . ' ' .  YIT_Contact_Form()->get('_submit_alignment'); ?>" />
                <?php wp_nonce_field( 'yit-sendmail' ); ?>
                <div class="clear"></div>
            </li>
        </ul>
    </fieldset>

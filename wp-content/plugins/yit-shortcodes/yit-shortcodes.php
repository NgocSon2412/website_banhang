...
class YIT_Shortcodes {
    ...
    */
     ...
     */
     public function media_buttons_context($context){
         ...
         $out = '<a id="add_shortcode" style="display:none" href="'.admin_url( 'admin.php?action=yit_shortcode_popup&post_id='.$iframe_ID.'&TB_iframe=1' ).'" class="hide-if-no-js thickbox" title="'. __("Add shortcode", 'yit').'"><img src="'.plugins_url('assets/images/icon_shortcodes.png', __FILE__).'" alt="'. __( "Add Shortcode", 'yit' ) . '" /></a>';
    ...
    */
    public function add_shortcodes_tinymce_plugin( $plugin_array ) {
        $plugin_array['yitshortcodes'] = plugins_url('assets/js/tinymce.js', __FILE__);
        return $plugin_array;
    }
    ...
    */
    public function shortcode_icon( $shortcode ){
        ...
        elseif( file_exists( dirname( __FILE__ ) . '/assets/images/' . $shortcode . '.png') ){
            $return = plugins_url( '/assets/images/', __FILE__ ) . $shortcode . '.png';
        }else {
            $return = plugins_url( '/assets/images/', __FILE__ ) . 'default-shortcode-icon.png';
        }
    ...
    */
    public function add_visual_composer_type(){
        ...
        foreach( $files as $file ){
            ...
            add_shortcode_param( $type, array( $this, 'print_vc_fields' ), $this->plugin_assets_url . "/js/visual-composer.js" );
        }
    }
